...
impl VimMotions {
    ...
    pub fn word_forward(&self, pos: usize) -> Option<usize> {
        ...
        // Skip current word
        while i < chars.len()
            && Self::is_word_char(chars[i]) == current_is_word
            && !Self::is_whitespace(chars[i])
        {
            i += 1;
        }
    ...
    pub fn word_backward(&self, pos: usize) -> Option<usize> {
        ...
        if i > 0
            && i < chars.len()
            && !Self::is_whitespace(chars[i])
            && (i == 0 || Self::is_whitespace(chars[i - 1]))
        {
            i -= 1;
        }
...
mod tests {
    ...
    fn test_execute_with_count() {
        ...
        assert_eq!(
            motions.execute_with_count(0, Motion::WordForward, 2),
            Some(8)
        );
        // 3w - three words forward
        assert_eq!(
            motions.execute_with_count(0, Motion::WordForward, 3),
            Some(14)
        );
    }
    ...
    fn test_execute_with_count_zero() {
        ...
        assert_eq!(
            motions.execute_with_count(0, Motion::WordForward, 0),
            Some(4)
        );
    }
