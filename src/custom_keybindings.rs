...
impl CustomKeybindings {
    ...
    pub fn set_description(&mut self, action: &str, description: impl Into<String>) {
        self.descriptions.insert(action.to_string(), description.into());
    }
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        let content = fs::read_to_string(path.as_ref())
            .context("Failed to read keybindings file")?;
        let bindings: Self = serde_json::from_str(&content)
            .context("Failed to parse keybindings JSON")?;
        Ok(bindings)
    }
    ...
    pub fn save_to_file<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        let json = serde_json::to_string_pretty(self)
            .context("Failed to serialize keybindings")?;
        fs::write(path.as_ref(), json)
            .context("Failed to write keybindings file")?;
        Ok(())
    }
...
mod tests {
    ...
    fn test_bind_global() {
        ...
        assert_eq!(bindings.get_in_context(KeybindingContext::Normal, &key), Some(&"cancel".to_string()));
        assert_eq!(bindings.get_in_context(KeybindingContext::Insert, &key), Some(&"cancel".to_string()));
    }
    ...
    fn test_unbind() {
        ...
        assert!(bindings.get_in_context(KeybindingContext::Global, &key).is_none());
    }
