...
impl GitGraph {
    ...
    fn render_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        for (i, commit) in display_commits.iter().enumerate() {
            ...
            // Branch name
            if self.show_branches && let Some(branch) = commit.branch() {
                spans.push(Span::styled(
                    format!("({}) ", branch),
            ...
            // Author
            if self.show_authors && let Some(author) = commit.author() {
                spans.push(Span::styled(
                    format!(" <{}>", author),
            ...
            if !self.compact && i < display_commits.len() - 1 && let Some(parent) = commit.parent()
