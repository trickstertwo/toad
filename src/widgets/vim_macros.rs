...
impl MacroRecorder {
    ...
    /// recorder.stop_recording();
    /// ```
    pub fn record_action_with_param(&mut self, action: impl Into<String>, param: impl Into<String>) {
        if self.is_recording() {
            self.recording_buffer.push(MacroAction::with_param(action, param));
        }
    }
...
impl MarkRegistry {
    ...
    /// assert!(marks.has_mark('A'));
    /// ```
    pub fn set_global_mark(&mut self, mark: char, file: impl Into<String>, position: (usize, usize)) {
        if mark.is_ascii_uppercase() {
            self.global_marks.insert(mark, (file.into(), position.0, position.1));
        }
    }
