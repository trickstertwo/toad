...
impl ScatterSeries {
    ...
    pub fn x_bounds(&self) -> (Option<f64>, Option<f64>) {
        let min = self.points.iter().map(|(x, _)| *x).min_by(|a, b| a.partial_cmp(b).unwrap());
        let max = self.points.iter().map(|(x, _)| *x).max_by(|a, b| a.partial_cmp(b).unwrap());
        (min, max)
    }
    ...
    pub fn y_bounds(&self) -> (Option<f64>, Option<f64>) {
        let min = self.points.iter().map(|(_, y)| *y).min_by(|a, b| a.partial_cmp(b).unwrap());
        let max = self.points.iter().map(|(_, y)| *y).max_by(|a, b| a.partial_cmp(b).unwrap());
        (min, max)
    }
}
