...
impl Canvas {
    ...
    pub fn point(&mut self, x: f64, y: f64, color: Color, marker: char) {
        self.shapes.push(Shape::Point { x, y, color, marker });
    }
    ...
    fn world_to_screen(&self, x: f64, y: f64, width: u16, height: u16) -> (u16, u16) {
        ...
        (screen_x.min(width.saturating_sub(1)), screen_y.min(height.saturating_sub(1)))
    }
    ...
    fn draw_line(&self, grid: &mut [Vec<char>], x1: f64, y1: f64, x2: f64, y2: f64, width: u16, height: u16) {
    ...
    #[allow(clippy::too_many_arguments)]
    fn draw_rectangle(&self, grid: &mut [Vec<char>], x1: f64, y1: f64, x2: f64, y2: f64, width: u16, height: u16) {
    ...
    /// Draw a filled rectangle
    #[allow(clippy::too_many_arguments)]
    fn draw_filled_rectangle(&self, grid: &mut [Vec<char>], x1: f64, y1: f64, x2: f64, y2: f64, width: u16, height: u16) {
        let (sx1, sy1) = self.world_to_screen(x1, y1, width, height);
        let (sx2, sy2) = self.world_to_screen(x2, y2, width, height);
    ...
    #[allow(clippy::too_many_arguments)]
    fn draw_circle(&self, grid: &mut [Vec<char>], cx: f64, cy: f64, radius: f64, width: u16, height: u16) {
    ...
    #[allow(clippy::too_many_arguments)]
    fn plot_circle_points(&self, grid: &mut [Vec<char>], cx: i32, cy: i32, x: i32, y: i32, width: u16, height: u16) {
        let points = [
            (cx + x, cy + y), (cx - x, cy + y),
            (cx + x, cy - y), (cx - x, cy - y),
            (cx + y, cy + x), (cx - y, cy + x),
    ...
    /// Draw a filled circle
    #[allow(clippy::too_many_arguments)]
    fn draw_filled_circle(&self, grid: &mut [Vec<char>], cx: f64, cy: f64, radius: f64, width: u16, height: u16) {
        let (scx, scy) = self.world_to_screen(cx, cy, width, height);
        let x_scale = width as f64 / (self.x_bounds.1 - self.x_bounds.0);
        let y_scale = height as f64 / (self.y_bounds.1 - self.y_bounds.0);
        let sr = ((radius * x_scale + radius * y_scale) / 2.0) as i32;
