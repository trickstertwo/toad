...
impl ErrorHandler {
    ...
    pub fn add_error(&mut self, error: ErrorEntry) {
        ...
        if let Some(existing) = self.errors.iter_mut().find(|e| {
            e.severity == error.severity && e.message == error.message
        }) {
...
mod tests {
    ...
    fn test_error_entry_with_details() {
        let error = ErrorEntry::new(ErrorSeverity::Warning, "Warning")
    ...
    fn test_error_entry_with_code() {
        let error = ErrorEntry::new(ErrorSeverity::Error, "Error")
            .with_code("E001");
