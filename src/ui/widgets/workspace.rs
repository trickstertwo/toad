...
impl WorkspaceManager {
    ...
    /// assert_eq!(manager.workspace_count(), 1);
    /// ```
    pub fn create_workspace(&mut self, name: impl Into<String>, path: impl Into<PathBuf>) -> bool {
        let name = name.into();
...
mod tests {
    ...
    fn test_workspace_manager_active_workspace_mut() {
        ...
        assert_eq!(
            manager.active_workspace().unwrap().get_setting("key"),
            Some("value")
        );
    }
