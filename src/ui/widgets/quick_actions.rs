...
mod tests {
    ...
    fn test_quick_action_with_keybind() {
        let action = QuickAction::new("Save", "Save file", "save").with_keybind("Ctrl+S");
        assert_eq!(action.keybind(), Some("Ctrl+S"));
    }

    #[test]
    fn test_quick_action_with_icon() {
        let action = QuickAction::new("Save", "Save file", "save").with_icon("💾");
        assert_eq!(action.icon(), Some("💾"));
    }

    #[test]
    fn test_quick_action_with_enabled() {
        let action = QuickAction::new("Paste", "Paste text", "paste").with_enabled(false);
        assert!(!action.is_enabled());
    }
    ...
    fn test_quick_actions_with_keybinds() {
        let actions = QuickActions::new().with_keybinds(false);
        assert!(!actions.show_keybinds);
    }

    #[test]
    fn test_quick_actions_with_icons() {
        let actions = QuickActions::new().with_icons(false);
        assert!(!actions.show_icons);
    }

    #[test]
    fn test_quick_actions_with_descriptions() {
        let actions = QuickActions::new().with_descriptions(false);
        assert!(!actions.show_descriptions);
    }

    #[test]
    fn test_quick_actions_with_title() {
        let actions = QuickActions::new().with_title("File Actions");
        assert_eq!(actions.title, Some("File Actions".to_string()));
    }

    #[test]
    fn test_quick_actions_with_compact() {
        let actions = QuickActions::new().with_compact(true);
    ...
    fn test_quick_actions_selected_item() {
        let actions = QuickActions::new().add_action("Save", "Save file", "save", None::<&str>);
