...
fn test_data_series_with_decimal_precision() {
    let series = DataSeries::new("Precision", vec![1.123456789, 2.987654321, 3.141592653]);
...
fn test_line_chart_render_with_10k_points() {
    ...
    let chart = LineChart::new().add_series(DataSeries::new("Sine Wave", data));
...
fn test_data_series_with_very_small_differences() {
    let series = DataSeries::new("Tiny Diffs", vec![1.0000001, 1.0000002, 1.0000003]);
    assert!(series.min().is_some());
    assert!(series.max().is_some());
}
...
fn test_line_chart_with_extreme_y_bounds() {
    let chart = LineChart::new().with_y_bounds(f64::MIN, f64::MAX);
...
fn test_line_chart_10_phase_comprehensive_workflow() {
    ...
    chart = chart.add_series(DataSeries::new("Final Series", vec![7.0, 8.0, 9.0]));
...
fn test_data_series_with_alternating_sign() {
    let series = DataSeries::new("Alternating", vec![-1.0, 1.0, -2.0, 2.0, -3.0, 3.0]);
...
fn test_data_series_with_exponential_values() {
    let series = DataSeries::new("Exponential", vec![1e-10, 1e-5, 1e0, 1e5, 1e10]);
