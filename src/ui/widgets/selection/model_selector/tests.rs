...
fn test_model_mixed_scripts_name() {
    let model = ModelInfo::new("test", "Hello世界Привет안녕하세요", "Provider");
...
fn test_model_context_window_max() {
    let model = ModelInfo::new("test", "Test", "Provider").with_context_window(usize::MAX);
    assert_eq!(model.context_window, usize::MAX);
}

#[test]
fn test_model_context_window_zero() {
    let model = ModelInfo::new("test", "Test", "Provider").with_context_window(0);
    assert_eq!(model.context_window, 0);
}

#[test]
fn test_model_max_output_extreme() {
    let model = ModelInfo::new("test", "Test", "Provider").with_max_output(usize::MAX);
...
fn test_formatted_context_millions() {
    let model = ModelInfo::new("test", "Test", "Provider").with_context_window(5_000_000);
...
fn test_formatted_context_hundreds() {
    let model = ModelInfo::new("test", "Test", "Provider").with_context_window(500);
...
fn test_selector_add_remove_navigate() {
    ...
    // Add a new model
    selector
        .add_model(ModelInfo::new("new-model", "New Model", "Provider").with_capability("test"));
...
fn test_comprehensive_model_selector_stress() {
    ...
    for i in 0..100 {
        ...
        let mut model = ModelInfo::new(format!("model-{}", i), name, format!("Provider{}", i % 5))
...
fn test_selector_state_after_model_replacement() {
    ...
    let new_models = vec![ModelInfo::new("m1", "Model 1", "P1")];
...
fn test_selector_state_after_model_expansion() {
    ...
    // Add more models
    for i in 0..50 {
        selector.add_model(ModelInfo::new(
            format!("m{}", i),
            format!("Model {}", i),
            "P",
        ));
...
fn test_filter_with_unicode_capability() {
    let mut selector = ModelSelector::new();
    selector.add_model(ModelInfo::new("test", "Test", "Provider").with_capability("日本語処理"));
...
fn test_model_info_clone_independence() {
    let mut model1 = ModelInfo::new("test", "Test", "Provider").with_capability("coding");
